...
async function boot(){try{loading();let testProfile=profiles.find(x=>x.test);if(!testProfile){testProfile={name:'BlackShark TEST',tag:'#22QYOQRGY',test:true};profiles=[testProfile,...profiles];saveProfiles()}let p=profiles.find(x=>x.name===active);if(!p||(!apiBase()&&!p.test)){p=testProfile;active=p.name;localStorage.setItem('bh_active',active)}else{active=p.name;localStorage.setItem('bh_active',active)}await loadProfile(p);Promise.allSettled([loadMeta(),loadCatalog()]).then(()=>{if(P)render()});if('serviceWorker'in navigator)navigator.serviceWorker.register('sw.js').catch(()=>{})}catch(e){showFatal(e)}}
