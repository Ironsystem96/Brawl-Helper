...
function brawlerGuide(b){const e=buildEntry(b),c=guideEntry(b),m=metaEntry(b,playMode,playMap),desc=cleanGuideText(c?.description||c?.shortDescription||''),rows=buildItems(b).map(([label,type,item])=>guideComponent(b,'Recommended '+label,type,item)).join(''),extras=(c?.overdrives||c?.overdrive)?guideComponent(b,'Overdrive','overdrives',first(c.overdrives||[c.overdrive])):'';return '<section class="card guideCard"><div class="sectionTitle"><h2>What this Brawler does</h2><span class="chip">'+esc(metaScope(b,playMode,playMap))+'</span></div><p class="guideDescription">'+esc(desc||'Description not available in the current catalog.')+'</p><div class="guideContext"><span>BEST CONTEXT</span><b>'+esc(m?'Recommended for '+playMode+(playMap!=='Random'?' · '+playMap:''):'Meta context not available for this selection.')+'</b></div><div class="guideGrid">'+rows+extras+'</div><div class="guideWhy"><b>Why this build?</b><p>'+esc(e?'Community build usage is the current build signal. '+(e.sample?fmt(e.sample)+' submitted builds were available.':''):'No verified community build is available yet.')+'</p></div><p class="small">Tap any component to open its details. Confirm ownership when the public profile cannot provide it.</p></section>'}
function buildAdvisor(b){const e=buildEntry(b);if(!e)return '<section class="card"><div class="sectionTitle"><h3>Recommended build</h3><span class="chip">DATA PENDING</span></div><p class="small">No verified community build is available for this Brawler yet. The app will not invent one.</p></section>';const rows=buildItems(b).map(([label,type,item])=>advisorRow(b,label,type,item)).join('');return '<section class="card"><div class="sectionTitle"><h3>Recommended build</h3><span class="chip ok">COMMUNITY DATA</span></div><p class="small">Status separates what the profile reports as equipped from what you manually confirm as owned or not owned.</p>'+rows+'<p class="small">Source: <a href="'+esc(e.sourceUrl||'#')+'" target="_blank" rel="noopener">community build data</a>'+(e.sample?' · '+fmt(e.sample)+' builds':'')+'</p></section>'}
...
function bcard(b,compact=false,mode=playMode,map=playMap,rank=null,kind=''){if(!b)return '';const st=status(b),m=metaEntry(b,mode,map),metaRank=rank||metaRankOf(b,mode,map),ownedAccount=isOwnedAccount(b),actions=nextActions(b);return '<div class="card bcard '+(kind==='recommended'?'recommendedCard':'')+'" onclick="openB('+b.id+')"><div class="row"><div class="rankBadge">'+(metaRank?'<span class="metaRank">#'+metaRank+'</span>':'•')+'</div><img class="portrait" src="'+portrait(b)+'" onerror="this.style.opacity=.25"><div class="grow"><div class="bname">'+esc(b.name)+'</div><div class="small bMetaLine">'+(ownedAccount?'P'+b.power+' · '+fmt(b.trophies)+' trophies':'NOT OWNED')+' · '+(m?'META':'META N/A')+'</div><div class="chips"><span class="chip '+st[1]+'">'+(ownedAccount?st[0]:'UNLOCK')+'</span><span class="chip">'+(actions.length?esc(actions[0].type):'READY')+'</span></div>'+miniBuild(b)+'</div></div>'+(!compact?'<div class="chips detailChips"><span class="chip">GADGET '+(b.gadgets?.length||0)+' equipped</span><span class="chip">STAR POWER '+(b.starPowers?.length||0)+' equipped</span><span class="chip">GEAR '+(b.gears?.length||0)+' equipped</span><span class="chip">HC '+(b.hyperCharges?.length||0)+' equipped</span></div>':'')+'</div>'}
function home(){const ownedList=[...(P?.brawlers||[])].filter(Boolean),plans=ownedList.map(b=>({b,plan:upgradePriority(b)})),upgrades=plans.filter(x=>x.plan.actions.some(a=>['BUY','POWER','EQUIP'].includes(a.type))).sort((a,z)=>z.plan.score-a.plan.score).slice(0,3);const quick='<section class="quickGrid"><button onclick="setTab(\'upgrade\')"><span class="quickIcon">↗</span><b>Upgrade</b><small>What next?</small></button><button onclick="setTab(\'brawlers\')"><span class="quickIcon">●</span><b>Brawlers</b><small>Browse builds</small></button><button onclick="tab=\'play\';render()"><span class="quickIcon">▶</span><b>Modes</b><small>Meta & maps</small></button></section>';const legend='<div class="stateLegend"><span><i class="legendDot equippedDot">✓</i> EQUIPPED</span><span><i class="legendDot ownedDot">●</i> OWNED</span><span><i class="legendDot buyDot">＋</i> BUY</span><span><i class="legendDot unknownDot">?</i> UNKNOWN</span></div>';return '<section class="hero"><div class="eyebrow">ACCOUNT</div><h1>'+esc(P.name)+'</h1><div class="tag">'+esc(P.tag)+' · Level '+P.expLevel+'</div><div class="stats"><div class="stat"><b>'+fmt(P.trophies)+'</b><span>TROPHIES</span></div><div class="stat"><b>'+P.brawlers.length+'</b><span>BRAWLERS</span></div><div class="stat"><b>'+fmt(P['3vs3Victories'])+'</b><span>3v3 WINS</span></div></div></section>'+quick+'<section class="section"><div class="sectionTitle"><h2>Next upgrades</h2><span class="small">'+(upgrades.length?'Top '+upgrades.length:'No confirmed actions')+'</span></div>'+legend+'<div class="notice upgradeIntro"><b>Important:</b> the profile can tell us equipped items, but not every item you own but do not have equipped. Confirm an item on its Brawler page to make BUY/EQUIP guidance precise.</div>'+(upgrades.length?upgrades.map((x,i)=>upgradeCard(x,i+1)).join(''):'<div class="card emptyState"><b>No confirmed upgrade actions.</b><span class="small">Open a Brawler to confirm missing items.</span></div>')+'</section><section class="section"><div class="sectionTitle"><h2>Explore</h2><span class="small">The guide lives inside each Brawler</span></div><button class="browseCta" onclick="setTab(\'brawlers\')">Open Brawler Browser <span>→</span></button></section>'}
function upgradeCard(x,rank){const b=x.b,plan=x.plan;return '<div class="upgradeCard" onclick="openB('+b.id+')"><div class="upgradeTop"><span class="priorityNum">'+rank+'</span><img class="upgradePortrait" src="'+portrait(b)+'" onerror="this.style.opacity=.25"><div class="grow"><b>'+esc(b.name)+'</b><span class="small">Power '+b.power+' · '+fmt(b.trophies)+' trophies</span></div><span class="priorityLabel">NEXT</span></div><div class="upgradeActions">'+plan.actions.map(a=>'<div class="priorityAction"><span class="priorityIcon">'+(a.type==='BUY'?'＋':a.type==='POWER'?'↑':a.type==='EQUIP'?'✓':'?')+'</span><div><b>'+esc(a.label)+'</b><span>'+esc(a.reason)+'</span></div></div>').join('')+'</div>'+miniBuild(b)+'</div>'}
function brawlers(){let bs=allBrawlers().filter(Boolean);if(query)bs=bs.filter(b=>b.name.toLowerCase().includes(query.toLowerCase()));if(filter==='p11')bs=bs.filter(b=>isOwnedAccount(b)&&b.power>=11);if(filter==='upgrade')bs=bs.filter(b=>isOwnedAccount(b)&&nextActions(b).some(a=>['BUY','POWER','EQUIP'].includes(a.type)));if(filter==='missing')bs=bs.filter(b=>isOwnedAccount(b)&&nextActions(b).some(a=>a.type==='BUY'||a.type==='CHECK'));bs.sort((a,z)=>(isOwnedAccount(z)-isOwnedAccount(a))||(z.trophies||0)-(a.trophies||0));return '<section class="section"><div class="sectionTitle"><h2>Brawler Browser</h2><span class="small">'+bs.length+' in catalog</span></div><div class="browserIntro"><b>Tap a Brawler to open the full guide.</b><span>Every card separates account status, equipped items, recommended build and unknown ownership.</span></div><input class="search" placeholder="Search Brawler..." value="'+esc(query)+'" oninput="query=this.value;render()"><div class="filters"><button class="'+(filter==='all'?'active':'')+'" onclick="filter=\'all\';render()">All</button><button class="'+(filter==='p11'?'active':'')+'" onclick="filter=\'p11\';render()">Power 11</button><button class="'+(filter==='upgrade'?'active':'')+'" onclick="filter=\'upgrade\';render()">Upgrade next</button><button class="'+(filter==='missing'?'active':'')+'" onclick="filter=\'missing\';render()">Needs review</button></div>'+bs.map(bcard).join('')+'</section>'}
function detail(){const b=P.brawlers.find(x=>x&&x.id===selected)||allBrawlers().find(x=>x&&x.id===selected);if(!b)return '';const accountOwned=isOwnedAccount(b),actions=nextActions(b),e=buildEntry(b),recommended=e?buildItems(b):[],equipped=[...(b.gadgets||[]).map(x=>['Gadget','gadgets',x]),...(b.starPowers||[]).map(x=>['Star Power','starPowers',x]),...(b.gears||[]).map(x=>['Gear','gears',x]),...(b.hyperCharges||[]).map(x=>['Hypercharge','hyperCharges',x])];return '<button class="back" onclick="selected=null;render()">← Back to Brawlers</button><section class="hero brawlerHero"><div class="row"><img class="portrait heroPortrait" src="'+portrait(b)+'"><div class="grow"><div class="eyebrow">BRAWLER GUIDE</div><h1>'+esc(b.name)+'</h1><div class="muted">'+(accountOwned?'Power '+b.power+' · '+fmt(b.trophies)+' trophies':'Not owned')+'</div></div></div></section><section class="card statusCard"><div class="sectionTitle"><h2>At a glance</h2><span class="chip">'+(actions.length?esc(actions[0].type):'READY')+'</span></div><div class="stateLegend"><span><i class="legendDot equippedDot">✓</i> Equipped</span><span><i class="legendDot ownedDot">●</i> Owned</span><span><i class="legendDot buyDot">＋</i> Buy</span><span><i class="legendDot unknownDot">?</i> Unknown</span></div><div class="actionStack">'+actions.slice(0,4).map(a=>'<div class="nextAction"><b>'+esc(a.label)+'</b><span>'+esc(a.reason)+'</span></div>').join('')+'</div></section><section class="card"><div class="sectionTitle"><h2>Your equipped setup</h2><span class="small">from profile</span></div><div class="equippedGrid">'+(equipped.length?equipped.map(([label,type,item])=>'<button class="equippedItem" type="button" onclick="componentModalById('+b.id+',\''+type+'\','+(item.id||0)+')">'+compIcon(type,item,b)+'<span><small>'+label+'</small><b>'+esc(item.name)+'</b></span><strong>EQUIPPED</strong></button>').join(''):'<div class="emptyState"><b>No equipped components reported.</b><span class="small">No selection was reported for this slot.</span></div>')+'</div></section><section class="card"><div class="sectionTitle"><h2>Recommended build</h2><span class="small">'+(e?'community data':'data pending')+'</span></div><p class="small">Confirm ownership for each recommended item when the profile cannot provide it. This prevents false BUY recommendations.</p><div class="recommendedList">'+(recommended.length?recommended.map(([label,type,item])=>'<div class="recommendRow">'+guideComponent(b,label,type,item)+ownershipControl(b,type,item)+'</div>').join(''):'<div class="emptyState"><b>Build data pending.</b><span class="small">No verified community build is available for this Brawler.</span></div>')+'</div></section>'+brawlerGuide(b)+buildAdvisor(b)+'<section class="card"><div class="sectionTitle"><h3>Progression map</h3><span class="small">verified vs manual</span></div><div class="progressMap"><div><b>Power</b><span>'+esc(String(b.power||0))+'/11</span></div><div><b>Equipped Gadgets</b><span>'+((b.gadgets||[]).length)+'</span></div><div><b>Equipped Star Powers</b><span>'+((b.starPowers||[]).length)+'</span></div><div><b>Equipped Gears</b><span>'+((b.gears||[]).length)+'</span></div><div><b>Equipped Hypercharges</b><span>'+((b.hyperCharges||[]).length)+'</span></div><div><b>Buffies</b><span>'+esc(b.buffies?Object.values(b.buffies).filter(Boolean).length:'—')+'</span></div></div></section>'}
function play(){const names=MODES[playMode]||MODES.Ranked,ranked=metaRankList(playMode,playMap),ready=ranked.filter(x=>isOwnedAccount(x.b)).slice(0,5);return '<section class="section"><div class="sectionTitle"><h2>Modes</h2><span class="small">'+esc(metaScope(ready[0]?.b,playMode,playMap))+'</span></div><div class="notice">'+(META_STATE.loaded?'Select a mode and map to see verified meta context.':'Meta data is not published yet. This page will become the mode/map recommendation surface once the validated snapshot is available.')+'</div><div class="filters">'+Object.keys(MODES).map(m=>'<button class="'+(playMode===m?'active':'')+'" onclick="playMode=\''+m+'\';playMap=\'Random\';render()">'+m+'</button>').join('')+'</div><select class="search" onchange="playMap=this.value;render()">'+names.map(m=>'<option '+(playMap===m?'selected':'')+'>'+m+'</option>').join('')+'</select>'+(ready.length?'<section class="section"><div class="sectionTitle"><h2>Your options</h2></div>'+ready.map(x=>bcard(x.b,true,playMode,playMap,x.meta.rank||metaRankOf(x.b,playMode,playMap),'ownedTop')).join('')+'</section>':'<div class="emptyState"><b>No verified mode meta yet.</b><span class="small">Use Brawler Guide and Upgrade until the snapshot is available.</span></div>')+'</section>'}

function upgrade(){const list=[...(P.brawlers||[])].filter(Boolean).map(b=>({b,plan:upgradePriority(b)})).filter(x=>x.plan.actions.length).sort((a,z)=>z.plan.score-a.plan.score);return '<section class="section"><div class="sectionTitle"><h2>Upgrade Queue</h2><span class="small">one task at a time</span></div><div class="notice upgradeIntro"><b>BUY</b> = manually marked not owned. <b>EQUIP</b> = manually marked owned but not equipped. <b>CHECK</b> = ownership still unknown.</div>'+(list.length?list.map((x,i)=>upgradeCard(x,i+1)).join(''):'<div class="card emptyState"><b>Queue is empty.</b><span class="small">Open a Brawler and confirm your items.</span></div>')+'</section>'}
function meta(){
 ...
 (META_STATE.loaded?'Source: '+esc(META_STATE.source||'snapshot')+' · Updated: '+esc(META_STATE.updatedAt||'—'):'The game database and official changelog are separate from personal account data.')+
 '</div>'+
