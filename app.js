...
function brawlers(){let bs=[...(P.brawlers||[])].filter(Boolean);if(query)bs=bs.filter(b=>b.name.toLowerCase().includes(query.toLowerCase()));if(filter==='p11')bs=bs.filter(b=>b.power>=11);if(filter==='upgrade')bs=bs.filter(b=>nextActions(b).some(a=>['BUY','POWER','EQUIP'].includes(a.type)));if(filter==='missing')bs=bs.filter(b=>nextActions(b).some(a=>a.type==='BUY'||a.type==='CHECK'));bs.sort((a,b)=>b.trophies-a.trophies);return '<section class="section"><div class="sectionTitle"><h2>Brawler Browser</h2><span class="small">'+bs.length+' shown</span></div><div class="browserIntro"><b>Each Brawler opens a complete guide.</b><span>Build icons show the recommended setup. Statuses separate EQUIPPED, OWNED, BUY and UNKNOWN.</span></div><input class="search" placeholder="Search Brawler..." value="'+esc(query)+'" oninput="query=this.value;render()"><div class="filters"><button class="'+(filter==='all'?'active':'')+'" onclick="filter=\'all\';render()">All</button><button class="'+(filter==='p11'?'active':'')+'" onclick="filter=\'p11\';render()">Power 11</button><button class="'+(filter==='upgrade'?'active':'')+'" onclick="filter=\'upgrade\';render()">Upgrade next</button><button class="'+(filter==='missing'?'active':'')+'" onclick="filter=\'missing\';render()">Needs review</button></div>'+bs.map(bcard).join('')+'</section>'}
